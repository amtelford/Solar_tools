...
## Defines an object containing the GUI with all its inputs, outputs and action buttons
    def __init__(self, master):  
        ...
        self.frameIn = Frame(master, bd=3, relief=SUNKEN) 
        self.frameIn.pack(side=LEFT, fill=BOTH, expand=1)
        
        ## Setup of latitude input
        Label(self.frameIn, text="Latitude [deg]: ").grid(row=0, column=0)
        self.latitude = Entry(self.frameIn)
        self.latitude.grid(row=0, column=1)
        
        ## Setup of longitude input
        Label(self.frameIn, text="Longitude [deg]: ").grid(row=1, column=0)
        self.longitude = Entry(self.frameIn)
        self.longitude.grid(row=1, column=1)
                
        ## Setup of power rating input
        Label(self.frameIn, text="Power rating [kW]: ").grid(row=2, column=0)
        self.Prating = Entry(self.frameIn)
        self.Prating.grid(row=2, column=1)
        
        ## Setup of area input
        Label(self.frameIn, text="Area [m2]: ").grid(row=3, column=0)
        ...
        Label(self.frameIn, text="Optimised for: ").grid(row=4, column=0)
        self.opt_selec = OptionMenu(self.frameIn, self.opt_setting, *opt_list)
        self.opt_selec.grid(row=4, column=1, sticky="ew") ## Sticky argument
        ## keeps the widget's width constant
        
        ## Setup of NREL API input
        Label(self.frameIn, text="NREL API: ").grid(row=5, column=0)
        self.api = Entry(self.frameIn)
        self.api.grid(row=5, column=1)
        
        ## Setup of CALCULATE button action
        self.calculate = Button(
            self.frameIn, text="CALCULATE", fg="black",
            font=("Times", 10, "bold"),
        ...
        self.frameOut = Frame(master, bd=3, relief=SUNKEN) 
        self.frameOut.pack(side=LEFT)

        ## Setup of output textbox
        self.data_present=False
        self.output = Text(self.frameOut)
        self.output.pack()            
...
## Function list:    
    ...
    ## Outputs the part of the dataframe entries in a textbox in the GUI.
        self.output.delete(1.0, END) ## Clears the textbox
        self.output.insert(END,"Month\t\t")
        self.output.insert(END,"PV output [kWh/day]")
        for i in range(0,12): ## Rows
            self.output.insert(END, "\n")
            for j in range(0,6,5): ## Columns
                if (j==0):
...
## Defines an object containing a dataframe with all the calculated data.
    def __init__(self):
        ...
        declination = [-20.8471721,
                     ...
                     -19.05050905,
                     -23.09560549]
        self.data = pd.DataFrame(np.transpose(month), columns=["Month"])
        self.data["Declination[deg]"] = pd.Series(declination, index=self.data.index)
