...
## buttons
    def __init__(self, master):  
        ...
        self.ctrlFrame = tk.Frame(master, bd=3, relief=tk.SUNKEN)
        self.ctrlFrame.pack(side=tk.LEFT, fill=tk.BOTH, expand=1)
		## Setup of Input frame
        self.frameIn = tk.Frame(self.ctrlFrame, 
                             bd=3, 
                             relief=tk.SUNKEN)
        self.frameIn.pack(side=tk.TOP, fill=tk.BOTH, expand=1)
	    ...
	    ## Setup of Comments frame
        self.comt = tk.Frame(self.ctrlFrame, 
                             bd=3, 
                             relief=tk.SUNKEN) 
        self.comt.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=1)       
        tk.Label(self.comt, text="\nOPTIMAL VALUES:\nTilt = latitude (Noththern hemisphere) or - latitude (Southern hemisphere)\nAzimuth =  180 (Noththern hemisphere) or 0 (Southern hemisphere)\n\nEXAMPLE: Sydney, Australia\nLat: -33.865143\nLong:151.2099\nTimezone: Australia/Sydney\nTilt: 33.865143 (optimal)\nAzimuth: 0 (optimal)\nAlbedo: 0.2 (city) ", anchor = 'w').grid(column=0, row=9)
		
        ## Setup of Output frame
        self.frameOut = tk.Frame(master, 
                             bd=3, 
                             relief=tk.SUNKEN) 
        self.frameOut.pack(side=tk.RIGHT,  fill=tk.BOTH, expand=1)
